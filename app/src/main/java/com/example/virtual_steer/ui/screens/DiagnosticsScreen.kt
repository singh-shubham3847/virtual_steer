...
import com.example.virtual_steer.ui.theme.AccentYellow
import com.example.virtual_steer.ui.theme.CarbonDark
import com.example.virtual_steer.ui.theme.ThrottleGreen
...
private fun PedalDiagCard(title: String, p: PedalDiagnostics) {
    TuningCard(title.uppercase()) {
        DiagRow("Touch Y", String.format(Locale.US, "%.1f px", p.rawTouchY))
        DiagRow("Normalized", String.format(Locale.US, "%.3f", p.normalized))
        HorizontalDivider(color = Color.White.copy(alpha = 0.1f), modifier = Modifier.padding(vertical = 8.dp))
        DiagRow("Curve Output", String.format(Locale.US, "%.3f", p.curveOutput))
        DiagRow("Smoothed", String.format(Locale.US, "%.3f", p.smoothed))
